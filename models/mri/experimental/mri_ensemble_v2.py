#!/usr/bin/env python3
"""
MRI Brain Tumor Classification Model using Ensemble (RandomForest + GradientBoosting + ExtraTrees + SVM)
...
class MRIEnsembleV2Classifier:
    def __init__(self, model_path='mri_ensemble_v2_model.pkl', dataset_path='../../datasets/mri scan'):
        self.model = None
    ...
    def train_ensemble_v2_model(self, max_images_per_class=1000):
        """Train Ensemble V2 model with optimized hyperparameters"""
        logger.info("Training Ensemble V2 MRI classification model...")
        ...
        # Feature selection
        self.feature_selector = SelectKBest(f_classif, k=150)
        ...
        logger.info(f"Gradient Boosting validation accuracy: {gb_score:.4f}")
        ...
        et_score = et_model.score(X_val, y_val)
        logger.info(f"Extra Trees validation accuracy: {et_score:.4f}")
...
def main():
    """Main function"""
    classifier = MRIEnsembleV2Classifier()
    
    # Train model
    training_accuracy = classifier.train_ensemble_v2_model(max_images_per_class=1000)
