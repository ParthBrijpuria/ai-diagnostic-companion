#!/usr/bin/env python3
"""
Improved MRI Brain Tumor Classification Model
Uses multiple algorithms to achieve higher accuracy for all 4 categories
"""
...
class ImprovedMRIClassifier:
    def __init__(self, model_path='mri_improved_model.pkl', dataset_path='datasets/mri scan'):
        self.model = None
    ...
    def extract_enhanced_features(self, images):
        ...
        for img_flat in images:
            ...
            try:
                ...
                features.extend([0] * 18)  # Increased for LBP features
    ...
    def train_ensemble_model(self, max_images_per_class=400):
        """Train an ensemble model for better accuracy"""
        logger.info("Training improved ensemble MRI model...")
        ...
        logger.info(f"Ensemble Model - Train Acc: {train_acc:.3f}, Val Acc: {val_acc:.3f}")
        
        # Train final model on full training set
        logger.info("Training final ensemble model on full training set...")
        ensemble_model.fit(train_features_scaled, train_labels)
        
        # Calculate final training accuracy
        train_pred_final = ensemble_model.predict(train_features_scaled)
    ...
    def evaluate(self, test_data_dir):
        ...
        for cls, acc in class_accuracy.items():
            formatted_name = 'No Tumor' if cls == 'notumor' else cls.capitalize()
            logger.info(f"  {formatted_name:<12}: {acc:.3f}")
        
        # Confusion matrix
        cm = confusion_matrix(test_labels, predictions, labels=self.classes)
        logger.info("Confusion Matrix:")
        logger.info(f"  {'Glioma':<12} {'Meningioma':<12} {'No Tumor':<12} {'Pituitary':<12}")
        for i, class_name in enumerate(self.classes):
            formatted_name = 'No Tumor' if class_name == 'notumor' else class_name.capitalize()
            logger.info(f"{formatted_name:<12} {cm[i]}")
...
def main():
    """Main function to train and evaluate the improved MRI model"""
    classifier = ImprovedMRIClassifier()
    
    # Train the model
    logger.info("Training improved MRI brain tumor classification model...")
    training_accuracy = classifier.train_ensemble_model(max_images_per_class=500)  # Use more data
    logger.info(f"Training Accuracy: {training_accuracy:.2%}")

    # Evaluate on the test set
    logger.info("Evaluating model on test set...")
    test_accuracy, class_wise_accuracy = classifier.evaluate(os.path.join(classifier.dataset_path, 'Testing'))
    logger.info(f"Overall Test Accuracy: {test_accuracy:.2%}")
    logger.info("Class-wise Test Results:")
    for cls, acc in class_wise_accuracy.items():
        formatted_name = 'No Tumor' if cls == 'notumor' else cls.capitalize()
        logger.info(f"  {formatted_name:<12}: {acc:.2%}")
