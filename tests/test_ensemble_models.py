#!/usr/bin/env python3
"""
Test the ensemble MRI and X-ray models
"""
...
def extract_mri_features_ensemble(images):
    """Extract features matching MRI ensemble model expectations"""
    features = []
...
def extract_xray_features_ensemble(images):
    """Extract features matching X-ray ensemble model expectations"""
    features = []
...
def test_mri_ensemble_model():
    """Test MRI ensemble model accuracy"""
    logger.info("🧠 Testing MRI Ensemble Model Accuracy...")
    logger.info("=" * 50)
    
    # Load model
    model_path = 'models/mri/mri_ensemble_model.pkl'
    if not os.path.exists(model_path):
        logger.error(f"MRI ensemble model not found: {model_path}")
        return None
    ...
    test_features = extract_mri_features_ensemble(test_images)
    ...
    logger.info(f"✅ MRI Ensemble Model Test Results:")
...
def test_xray_ensemble_model():
    """Test X-ray ensemble model accuracy"""
    logger.info("🫁 Testing X-ray Ensemble Model Accuracy...")
    logger.info("=" * 50)
    
    # Load model
    model_path = 'models/xray/xray_ensemble_model.pkl'
    if not os.path.exists(model_path):
        logger.error(f"X-ray ensemble model not found: {model_path}")
        return None
    ...
    # Extract features
    logger.info("Extracting features...")
    test_features = extract_xray_features_ensemble(test_images)
    ...
    logger.info(f"✅ X-ray Ensemble Model Test Results:")
    logger.info(f"   Test Accuracy: {accuracy:.4f} ({accuracy*100:.2f}%)")
    logger.info(f"   Test Samples: {len(test_images)}")
    logger.info(f"   Classes: {class_names}")
...
def main():
    ...
    # Test MRI Ensemble Model
    try:
        mri_accuracy = test_mri_ensemble_model()
        results['MRI Ensemble'] = mri_accuracy
    except Exception as e:
        logger.error(f"MRI ensemble model test failed: {e}")
        results['MRI Ensemble'] = None
    
    print("\n" + "=" * 60)
    
    # Test X-ray Ensemble Model
    try:
        xray_accuracy = test_xray_ensemble_model()
        results['X-ray Ensemble'] = xray_accuracy
    except Exception as e:
        logger.error(f"X-ray ensemble model test failed: {e}")
        results['X-ray Ensemble'] = None
    
    # Summary
    print("\n" + "=" * 60)
    print("📈 ENSEMBLE MODELS RESULTS SUMMARY")
    print("=" * 60)
    ...
    print("   Ensemble models should show significantly better performance")
