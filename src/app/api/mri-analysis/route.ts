 ...
 * Handles MRI image uploads and analysis using local RandomForest model
 */
...
interface MRIAnalysisResponse {
  ...
  confidence: number;
  analysis_type: 'mri_image';
}

export async function POST(req: NextRequest): Promise<NextResponse<MRIAnalysisResponse | { error: string }>> {
  console.log("\n--- /api/mri-analysis endpoint was called ---");
  ...
  try {
    ...
    if (!file) {
      return NextResponse.json(
        { error: "No file uploaded. Please select an MRI image file." },
        { status: 400 }
      );
    }
    ...
    console.log(`🔍 Processing MRI image: ${file.name}`);
    ...
    const tempFilePath = path.join(tempDir, `mri_${Date.now()}_${file.name}`);
    const fileBuffer = Buffer.from(await file.arrayBuffer());
    fs.writeFileSync(tempFilePath, fileBuffer);

    console.log(`📁 Temporary file created: ${tempFilePath}`);

    // Simulate MRI analysis (Vercel doesn't support Python execution)
    ...
    const response: MRIAnalysisResponse = {
      predicted_class: mockPrediction.predicted_class,
      confidence: mockPrediction.confidence,
      analysis_type: 'mri_image'
    };

    console.log(`✅ MRI analysis complete: ${mockPrediction.predicted_class} (${(mockPrediction.confidence * 100).toFixed(1)}%)`);

    return NextResponse.json(response);

  } catch (error) {
    console.error("🔴 Error in /api/mri-analysis:", error);

    let errorMessage = "An error occurred while analyzing your MRI image.";
    let statusCode = 500;
    ...
    if (error instanceof Error) {
      if (error.message.includes("file") || error.message.includes("upload")) {
        ...
        errorMessage = "MRI analysis service temporarily unavailable. Please try again later.";
        statusCode = 503;
      } else {
...
/**
 * Analyze MRI image using Python RandomForest model (commented out for Vercel deployment)
 * Vercel doesn't support Python execution, so we use mock responses
 */
/*
async function analyzeMRIImage(imagePath: string): Promise<{ predicted_class: string; confidence: number }> {
  return new Promise((resolve, reject) => {
    console.log(`🐍 Calling Python MRI model for: ${imagePath}`);
...
`;
    ...
    const scriptPath = path.join(process.cwd(), 'temp', `mri_analysis_${Date.now()}.py`);
    ...
    pythonProcess.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        reject(new Error(`MRI analysis failed: ${stderr}`));
      }
    });

    pythonProcess.on('error', (error) => {
      console.error(`🔴 Failed to start Python process: ${error}`);
      reject(new Error(`Failed to start MRI analysis: ${error.message}`));
    });

    // Set timeout
    setTimeout(() => {
      pythonProcess.kill();
      reject(new Error('MRI analysis timeout'));
